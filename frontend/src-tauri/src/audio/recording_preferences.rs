...
pub fn get_default_recordings_folder() -> PathBuf {
    ...
    {
        // Windows: %USERPROFILE%\Music\atman-journal-recordings
        if let Some(music_dir) = dirs::audio_dir() {
            music_dir.join("atman-journal-recordings")
        } else {
            // Fallback to Documents if Music folder is not available
            dirs::document_dir()
                .unwrap_or_else(|| PathBuf::from("."))
                .join("atman-journal-recordings")
        }
    }

    #[cfg(target_os = "macos")]
    {
        // macOS: ~/Movies/atman-journal-recordings
        if let Some(movies_dir) = dirs::video_dir() {
            movies_dir.join("atman-journal-recordings")
        } else {
    ...
    {
        // Linux/Others: ~/Documents/atman-journal-recordings
        dirs::document_dir()
            .unwrap_or_else(|| PathBuf::from("."))
            .join("atman-journal-recordings")
    }
