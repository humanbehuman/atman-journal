...
/// - macOS: ~/Library/Application Support/AtmanJournal/templates/
/// - Windows: %APPDATA%\AtmanJournal\templates\
/// - Linux: ~/.config/AtmanJournal/templates/
fn get_custom_templates_dir() -> Option<PathBuf> {
    let mut path = dirs::data_dir()?;
    path.push("AtmanJournal");
    path.push("templates");
    Some(path)
}
